...
// Setters/Getters
PlayerTracker.prototype.shootToxic = function(){
	
	if(this.toxicNumber > 0 && this.toxicNumber != 0) {
		
		this.toxicNumber--;
		this.gameServer.shootToxicVirus(this);
		
	};
	
};

PlayerTracker.prototype.onPowerPlant = function(){
	
	var packetHandler = this.socket.packetHandler;
	
	this.numberOfSec += 5;
	
	packetHandler.sendPowersUpdate();
	
};

PlayerTracker.prototype.boostMe = function(){
	if(!this.haveBooster) return;
	if(this.boostEnable) return;
	...
	for(var i = 0; i < this.cells.length; i++){
		...
		this.cells[i].getSpeed = this.cells[i].getSpeedBoosted;
	...
	var inter = setInterval(function(){
		...
		if(me.numberOfBooster == 0){
			
			for(var i = 0; i < me.cells.length; i++){
		
				me.cells[i].getSpeed = me.cells[i].getSpeedBasic;
				
			};
			
			packetHandler.sendPowersUpdate();
			me.boostEnable = false;
