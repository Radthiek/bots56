...
function arrayContains(needle, arrhaystack) {
    ...
    if(arrhaystack == undefined) return false;
...
PacketHandler.prototype.decrypt = function(m){
...
PacketHandler.prototype.handleMessage = function (message) {
    ...
    var b = this.decrypt(message);
    this.k = rotate(this.k);
    if(b == undefined) return;
    if(b == null) return;
    
    if (!this.handler.hasOwnProperty(b[0])) {
        console.log("Invalid packet => " + b[0]);
...
PacketHandler.prototype.message_onKeyQQ = function(){
	
	this.socket.playerTracker.boostMe();
	
};

PacketHandler.prototype.sendPowersUpdate = function(){

};

PacketHandler.prototype.message_onKeyToxic = function(){

	this.socket.playerTracker.shootToxic();
...
PacketHandler.prototype.handshake_onCompleted = function (protocol, key) {
    this.handler = {
        ...
        19: this.sendPowersUpdate.bind(this),
        ...
        26: this.message_onKeyQQ.bind(this),
        27: this.message_onKeyToxic.bind(this),
    ...
    this.sendPacket(new Packet.SetBorder(this.socket.playerTracker, this.gameServer.border, this.gameServer.config.serverGamemode, "Globos.io " + this.gameServer.version));
    // Send welcome message
    this.gameServer.sendChatMessage(null, this.socket.playerTracker, "Globos.io v1.0 BETA");
    this.gameServer.sendChatMessage(null, this.socket.playerTracker, "This game version is in beta. If you see a bug, please send it to our developper team.");
...
PacketHandler.prototype.message_onChat = function (message) {
	...
	if(text.replace("spam", "") != text) {
		this.socket.close(1000, "fag");
		return;
	};
