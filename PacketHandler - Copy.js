...
PacketHandler.prototype.handleMessage = function (message) {
    ...
    this.key = rotate(this.key);
    if(message == undefined) return;
    if(message == null) return;
    
    if (!this.handler.hasOwnProperty(message[0])) {
        console.log("Invalid packet => " + message[0]);
        // this.socket.close(1002, "Do you think you will bypass this?");
        return;
    }
...
PacketHandler.prototype.handshake_onKey = function (message) {
	...
	setTimeout(function(){
		...
		if(that.gotAreply == false){
			
			that.socket.close(1002, "I am done with you. Take your bots and go away. - Peacefull.");
...
PacketHandler.prototype.isABot = function (message) {
	this.gotAreply = true;
    var reader = new BinaryReader(message);
	reader.skipBytes(1);
	
	var check = reader.readInt32();
	var check2 = reader.readDouble();
	var check3 = reader.readInt16();
	var check4 = reader.readInt16();
	
	Logger.info("Check: " + check + " Check2: " + check2 + " Check3: " + check3 + " Check4: " + check4);

	if(check == 21109005 && check2 == 256460121106805 && check3 == -32717 && check4 == 15){
		
		Logger.info("This user is not a bot.");
		this.handshake_onCompleted(this.handshakeProtocol, this.handshakeKey);
		
	}else{
		
		Logger.warn("Bot detected!");
		this.socket.close(1002, "Nice try! Do you like apples?");
